...
import { decimalFormatter, generateLongId } from '../../../scripts/misc/stringFormatter';
...
const ForgePopupComponent = () => {
    ...
    useEffect(() => {
        if (addItemIndexState == null || store.getState().addItemTypeState.value != "forge_popup"){
            clearItemIndex();
        }
    }, []);
        ...
        if (addItemIndexState != null && addItemIndexState.length <= 3){
            for(let i = 0; i < addItemIndexState.length; i++){
                if (addItemIndexState[i] != null && Array.isArray(addItemIndexState)){
                    dispatch({type: 'edit/addItemFilterReducer/SET', value: addItemIndexState[i].nft_type+"-"+Number(addItemIndexState[i].nft_stars)});
    ...
    return(
        <div className={`${style.popup_content_root}`} style={{width:"auto"}}>
            ...
            <div style={{display:"flex", justifyContent:"center", gap:"15px", alignItems:"center"}}>
                                ...
                                <span>
                                    {
                                        addItemIndexState != null && addItemIndexState[i] != null ?
                                            <ItemBlockComponent no_5_stars={true} inventory_popup={true} data={addItemIndexState[i]} add_index={i}/>
                                        :
                                            <AddItemBlockComponent no_5_stars={true} no_borders={true} type={"node"} add_index={i}/>
                                    }
                                </span>
                            )
                        }
                ...
                <div className={currentStar == 0 ? style.empty_card : style.null}>
                    <div style={{opacity: currentStar == 0 ? 0 : 1}}>
                        <ItemBlockComponent type={store.getState().addItemFilterState.value != null ? store.getState().addItemFilterState.value.split("-")[0] : null} forge={true} mint={true} custom_stars={currentStar + 1} hover={false}/>
                    </div>
                </div>
            </div>
            <div className={`${style.colored_button} ${style.red_button}`} style={{marginTop:"15px"}}
                onClick={() => {
                    clearItemIndex();
                }}
            >
            ...
            <p style={{color:"rgba(255, 255, 255, 0.5)", marginTop:"5px"}}>
                If you continue with forging, the 3 base NFTs will be burned and a new one will be minted to your inventory.
            </p>
            <div className={`${style.colored_button} ${style.grey_button}`} style={{marginTop:"15px", justifyContent:"center", height:"50px"}}
                onClick={() => {
                    if ((() => {
                        ...
                        for(let i = 0; i < addItemIndexState.length; i++){
                            ...
                            long_ids.push(addItemIndexState[i].nft_long_id);
                        }
                        
                        CLTRNFTContract.methods.cltrnft_forge(
                            long_ids[0],
                            long_ids[1],
                            long_ids[2],
                            generateLongId(),
                        ...
                        .on('transactionHash', (hash: any) => {
                            (async () => {
                                await forgeNFT({
                                    type: store.getState().addItemFilterState.value.split("-")[0],
                                    nft_ids: ids,
                                    long_id: generateLongId()
                                });
            ...
            >
                <img src={"./images/svgs/clettr-token.svg"} alt="clettr-logo" style={{width:"20px", marginRight:"5px"}}/>
                <p>
                    {
                        currentStar == 1 ?
                            99.99
                        ...
                        :
                            0
                    } <span style={{margin:"0px 5px", color:"rgba(255, 255, 255, 0.5)"}}>{decimalFormatter(store.getState().tickerPriceState.value != null ? (
                        currentStar == 1 ?
                            99.99
                        :
                        currentStar == 2 ?
                        ...
                        :
                            0
                    ) * store.getState().tickerPriceState.value : 0)}</span><span style={{color:"rgba(255, 255, 255, 0.5)"}}>{store.getState().currentCurrencyState.value.toUpperCase()}</span>
                </p>
            </div>
            <p style={{width:"100%", textAlign:"center", color:"rgba(255, 255, 255, 0.5)", marginTop:"15px"}}>

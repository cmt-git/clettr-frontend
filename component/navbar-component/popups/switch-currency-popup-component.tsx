...
const SwitchCurrencyPopupComponent = () => {
    ...
    return(
        <div className={style.popup_content_root} style={{maxWidth:"450px", width:"100vw"}}>
            ...
            <div className={style.select_box}>
                <select onChange={(e: any ) => setSelectedCurrency(e.target.value)}>
                    <option selected={currentCurrencyState == "php" ? true : false}>Philippine Peso (PHP)</option>
                    <option selected={currentCurrencyState == "usd" ? true : false}>United States Dollar (USD)</option>
                </select>
            ...
            <div className={`${style.colored_button} ${style.grey_button}`} style={{marginTop:"15px"}}
                onClick={() => {
                    const filtered_currency = (
                        selectedCurrency == "Philippine Peso (PHP)" ? "php" : "usd"
                    );

                    dispatch({type: 'edit/currentCurrencyReducer/SET', value: filtered_currency});

...
const RegisterQRCodePopupComponent = (props: any) => {
    ...
    useEffect(() => {
        const secret = speakeasy.generateSecret({
            name: `Clettr | ${queryState.user != null ? queryState.user.username : props.username}`
        });
    ...
    return(
        <div className={style.popup_content_root} style={{width:"100vw", maxWidth:"500px"}}>
            ...
            <div style={{width:"100%", justifyContent:"center", alignItems:"center", display:"flex", flexDirection:"column"}}>
                <img src={qrImg} style={{height:"150px", width:"150px", borderRadius:"6px", backgroundColor:"white"}}/>
                <p style={{marginTop:"20px"}}>
                    Setup 2FA (Two-factor Authentication)
                </p>
            </div>
            <p style={{color:"#80808E", marginTop:"10px", textAlign:"center", fontSize:"13px"}}>
                Please scan the QR code with your authentication app on mobile phone. This step is necessary to make your account more secure!
            </p>
            <p style={{fontSize:"13px", width:"100%", marginBottom:"10px", marginTop:"20px"}}>
                Authentication Code
            </p>
            <div className={style.input_box} style={{marginBottom:"20px"}}>
                <input placeholder="Code" ref={codeRef} maxLength={6}/>
            </div>
            <div className={`${style.grey_button} ${style.colored_button}`} onClick={() => register2FA({"secret": secretCode,"authentication_code": codeRef.current.value})}>
                Confirm
            </div>
            {
                props.type == 1 ?
                    [
                        <p style={{color:"#80808E", marginTop:"30px", marginBottom:"20px", textAlign:"center", fontSize:"13px"}}>
                            ⚠️  Warning skipping this step will make your account least secure!
                        </p>,
                        <div className={`${style.red_button} ${style.colored_button}`} onClick={() => {
                            link_messageBoxShow("Account has been registered, Please log in.", true)
                            closePopup();
                        }}>
                            Skip
                        </div>

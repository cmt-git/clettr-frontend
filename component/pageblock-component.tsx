...
const PageBlockComponent = (props: any) => {
  ...
  const getPagesValues = () => {
    ...
    if (Initialized == true) {
      if (store.getState().addItemFilterState.value == null) {
        props.query({ variables: { page: currentPage, ...props.variables } });
      } else {
        props.query({
          variables: {
            page: currentPage,
            filters: store.getState().addItemFilterState.value,
            ...props.variables,
          },
        });
      }
  ...
  return (
    <div
      className={`${style.page_block_component_root} ${style.light_grey_info_block}`}
      style={{ marginTop: "30px" }}
    >
      <p style={{ marginLeft: "10px", fontSize: "13px" }}>Page</p>
      <div
        className={style.numbers_body}
      ...
      >
        ...
        >
          <path
            d="M9.636,1.481a1,1,0,0,1,1.728,0L20.123,16.5a1,1,0,0,1-.864,1.5H1.741a1,1,0,0,1-.864-1.5Z"
          ...
          {pages != null
            ? pages.map((value: any, index: any) => {
                return (
                  <p
                    className={
                      currentPage != value ? style.unselected : style.null
                    }
                    style={{
                      // -> needs to be synced with getPagesValue
                      visibility:
                        _width + 100 < 60 * index + 1 ? "hidden" : "visible",
                      position:
                        _width + 100 < 60 * index + 1 ? "absolute" : "relative",
                    }}

...
import { calculateActiveBoost, calculatePassiveBoost, getComparedHash } from '../../../scripts/misc/compareHash';
...
import { changePopup, closePopup } from '../../popup-component/popup-container-component';
...
const PlayPopupComponent = () => {
    ...
    useEffect(() => {
        if (addItemIndexState == null || store.getState().addItemTypeState.value != "play_popup"){
            let init_arr: any = [null]

            store.dispatch({type: 'edit/addItemIndexReducer/SET', value: init_arr});
        };
        
        store.dispatch({type: 'edit/addItemTypeReducer/SET', value: "play_popup"});
    ...
    return(
        <div className={style.popup_content_root} style={{width:"auto"}}>
            ...
            <div style={{display:"flex", alignItems:"center", gap:"15px", height:""}}>
                {
                    <span>
                        {
                            addItemIndexState != null && addItemIndexState[0] != null ?
                                <ItemBlockComponent no_borders={true} inventory_popup={true} data={addItemIndexState[0]} add_index={0}/>
                            :
                    ...
                    queryState.user_set != null ?
                        (() => {
                            ...
                            for(let i = 0; i < queryState.user_set.user_set.length; i++){
                                set_items_arr.push(
                                    <span>
                                       <ItemBlockComponent hover={false} no_borders={true} data={queryState.user_set.user_set[i]}/>
                                    </span>
                                )
                            }
            ...
            <div className={`${style.grey_button} ${style.colored_button}`}
                ...
                onClick={() =>{
                    ...
                    changePopup(<InventoryPopupComponent add_index={0} not_user={true}/>);
                }}
            >
                Use Community Node
            </div>
            {
                addItemIndexState != null && addItemIndexState[0] != null && addItemIndexState[0].nft_type == "passive" ?
                    <div className={style.requirement_box} style={{marginTop:"15px"}}>
                        <div className={style.grey_info_block} style={{borderBottomLeftRadius:"0px", borderBottomRightRadius:"0px"}}>
                            <p style={{color:"rgba(255, 255, 255, 0.5)"}}>
                                Requirements
                            </p>
                        </div>
                        <div className={style.black_info_block} style={{borderTopLeftRadius:"0px", borderTopRightRadius:"0px"}}>
                            {

                                (() => {
                                    const requirements = addItemIndexState[0].nft_requirement.split("-");
                                    const block_arr = []
                                    ...
                                    for(let i = 0; i < requirements.length; i++){
                                        block_arr.push(
                                            <div className={style.light_black_info_block}>
                                                <p style={{color:"rgba(255, 255, 255, 0.5)"}}>
                                                    {
                                                        ["pink", "purple", "blue", "teal", "lime", "green", "yellow", "orange", "red"].includes(requirements[i]) ?
                                                            "Color"
                                                        :
                                                        ["striped", "spotted", "zigzag", "checkered", "cross", "sharp"].includes(requirements[i]) ?
                                                            "Pattern"
                                                        :
                                                ...
                                                <p>
                                                    {
                                                        requirements[i][0].toUpperCase() + requirements[i].substring(1, requirements[i].length)
                                                    }
                                                </p>
                                            </div>
                                        )
            ...
            <div style={{display:"flex", gap:"15px", alignItems:"center", margin:"15px 0px"}}>
                <div className={style.grey_info_block}
                    style={{display:"flex", justifyContent:"space-between"}}
                >
                    <p style={{color:"rgba(255, 255, 255, 0.5)"}}>
                ...
                >
                    ...
                    <p>
                        {
                            getComparedHash(setHash, "active")
                        }
                    </p>
                </div>
            </div>  
            <div style={{display:"flex", gap:"15px", alignItems:"center", margin:"15px 0px"}}>
                ...
                >
                    ...
                    <div style={{display:'flex', justifyContent:"center", alignItems:"center"}}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="40.546" height="58.389" viewBox="0 0 40.546 58.389"
                        ...
                        <p style={{opacity:0.5, width:"130px", textAlign:"left", marginRight:"-12px"}}>
                ...
                >
                    ...
                    <p className={style.yellow_text}>
                        {
                            addItemIndexState != null && addItemIndexState[0] != null && queryState.user_set != null ?
                                `x${calculateActiveBoost(setHash) + calculatePassiveBoost(addItemIndexState[0].nft_hash)} Boost`
                            :
                                "--- Boost"
                        }
                    </p>
            ...
            <div className={`${style.light_grey_button} ${style.colored_button}`}
                style={{marginTop:"15px"}}
                onClick={() => {
                    ...
                    if (queryState.user_info.node_used != null){
                        user_info_node_history_split = queryState.user_info.node_used.split("-");
                    }
                    ...
                    for(let i = 0; i < user_info_node_history_split.length; i++){
                        if (user_info_node_history_split[i] != null){
                            ...
                            if (ids.includes(user_info_node_history_split[i]) == false){
                                ...
                                if (increment == 7 && addItemIndexState[0].id == user_info_node_history_split[i] && addItemIndexState[0].current_owner != queryState.user.username){
                                    allow_play = false;
                                    reason = "You have used this community node seven times for this week.";
                                }
                            }
                    ...
                    for(let i = 0; i < node_req.length; i++){
                        for(let x = 0; x < queryState.user_set.user_set.length; x++){
                            const current_traits = queryState.user_set.user_set[x].nft_traits.split("-");
                            for(let z = 0; z < current_traits.length; z++){
                                ...
                                if (node_req[i] == Number(queryState.user_set.user_set[x].nft_stars)){
                                    const index = node_req.indexOf(node_req[i]);
                                    if (index > -1) {
                                        node_req.splice(index, 1); // 2nd parameter means remove one item only
                                    }
                    ...
                    if (queryState.user_info.current_energy > 0 && allow_play){
                        dispatch({type: 'edit/playStateReducer/SET', value: {
                            boost: calculateActiveBoost(setHash) + calculatePassiveBoost(addItemIndexState[0].nft_hash),
                        ...
                        if (queryState.user_info.current_energy <= 0){
                            link_messageBoxShow("You have no energy left. Please continue once it has been refilled.", false);
                        }
                        else {

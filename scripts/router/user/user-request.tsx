...
import { changePopup, closePopup } from "../../../component/popup-component/popup-container-component";
...
export const connectWallet = async (changePopup: any, registerPopup: any) => {
    ...
    if ((networkId == 97 || networkId == 5777) && bsc_address != null && signature != null){
...
export const ettrSubtract = async (json: any) => {
    await axiosInstance.post('/user/ettr_subtract', {
        "value": json.value
    }).then(res => {
...
export const validateAccount = async (json: any) => {
    await axiosInstance.post('/user/validate', {
...
export const requestAuthCode = async (email: string) => {
    ...
    await axiosInstance.post('/authentication/new', {
        "email": email,
...
export const requestEmailCode = async (json: any) => {
    await axiosInstance.post('/authentication/email_code', {
        "email": json.email,
...
export const register2FA = async (json: any) => {
    const data = JSON.parse(localStorage.getItem("user-instance") || '{}');
    await axiosInstance.post('/authentication/2FA/new', {
        "bsc_address": data.bsc_address,
        "signature": data.signature,
        "secret": json["secret"],
        "authentication_code": json["authentication_code"]
...
export const revoke2FA = async (json: any) => {
    const data = JSON.parse(localStorage.getItem("user-instance") || '{}');
    await axiosInstance.post('/authentication/2FA/revoke', {
        "authentication_code": json["authentication_code"],
        "authenticator_code": json["authenticator_code"]
    }).then(res => {
        link_messageBoxShow(res.data["message"], res.data["success"]);
...
export const registerAccount = async (json: any) => {
    await axiosInstance.post('/user/register', {
        ...
        if (res.data["success"]){
            changePopup(<RegisterQRCodePopupComponent username={json["username"]} type={1}/>);
        }
    })
}
...
export const revokeSession = async (id: number) => {
    await axiosInstance.post('/user/session_revoke', {
        id: id
    }).then(res => {
        if (res.data["success"]){
            window.location.reload();
...
export const updateSet = async (json: any) => {
    await axiosInstance.post('/user/updateset', {

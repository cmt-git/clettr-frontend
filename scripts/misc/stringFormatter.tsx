export const decimalFormatter = (value: Number) => {
    ...
    return parts[0].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + (parts[1] ? "." + Number(parts[1]).toString().substring(0, 4) : "");
}
...
export function timeConverter(UNIX_timestamp: number){
    ...
    var hour = (a.getHours().toString().length == 1 ? "0" : "") + (a.getHours() + 1);
...
export const generateLongId = () => {
    ...
    for(let i = 0; i < characters.length; i++){
        long_id += characters[Math.floor(Math.random() * characters.length)];
    }
    return long_id;
}